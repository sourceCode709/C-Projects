 ...
 * Term project
 * 
 * Implementation of the Shortest-Job-First SJF CPU process scheduling algorithm.
...
/* declare variables */
struct node *task_queue = NULL, *next_task, *temp, *min_task; 
...
Task *pickNextTask(){
    /* select task with shortest burst in list */
    ...
    while (temp != NULL){
        ...
        if (min_task->task->burst > next_task->task->burst){
            min_task = next_task;
        }
    }
