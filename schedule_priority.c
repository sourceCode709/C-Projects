 ...
 * Term project
 * 
 * Implementation of the non-preempted Priority CPU process scheduling algorithm.
...
/* declare variables */
struct node *task_queue = NULL, *next_task, *temp, *priority_task; 
...
Task *pickNextTask(){
    ...
    temp = next_task->next;
    priority_task = next_task;

    while (temp != NULL){
        next_task = temp;
        temp = temp->next;

        if (priority_task->task->priority < next_task->task->priority){
            priority_task = next_task;
        }
    }
